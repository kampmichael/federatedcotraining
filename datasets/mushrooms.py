...
class mushrooms(Dataset):  
    def __init__(self, method, num_clients, num_unlabeled = 0, num_samples_per_client = -1, test_proportion = 0.33,pickle_files=True):
        super(mushrooms, self).__init__(method, num_clients, num_unlabeled, num_samples_per_client)
        df = pd.read_csv("data/mushrooms.csv")
        #Encoding Categorical Features
        encoder = LabelEncoder()
        for i in df.columns:
            df[i] = encoder.fit_transform(df[i]) 
        #veil-type has only one value and hence is insignificant
        ...
        X_train,X_test,y_train,y_test = train_test_split(X_scaled, y, test_size=0.2, shuffle=True, random_state=0, stratify=y)
